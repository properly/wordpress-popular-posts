...
Text Domain: wordpress-popular-posts-post-array
Domain Path: /lang/
Network: false
...
if ( !class_exists('WordpressPopularPosts') ) {
	...
	class WordpressPopularPosts extends WP_Widget {
      ...
      'post_formats' => '',
        ...
        $available = array('video', 'audio', 'image');
        $post_formats = explode(',', $instance['post_formats']);

        // if standard selected, must exclude other
        $exclude = in_array('standard', $post_formats);
        $tax_query = ($exclude) ? $available : array();
        ...
        foreach($post_formats as $format) {
          ...
          if ($format != 'standard') {
            if ($exclude) {
              array_splice($tax_query, array_search($format, $tax_query), 1);
            } else {
              $tax_query[] = $format;
            }
          }
        ...
        //get ids
        $format_query = "SELECT term_id FROM wp_terms WHERE slug IN({$q})";
        $result_formats = $wpdb->get_results($format_query, ARRAY_A);
        $ids = array();

        foreach($result_formats as $r) {
          $ids[] = $r['term_id'];
        };
        ...
        $ids_str = implode(',', $ids);
				...
				if (!$exclude) {
					$where .= " AND p.ID IN (
						...
						FROM {$wpdb->term_relationships} AS r
							 JOIN {$wpdb->term_taxonomy} AS x ON x.term_taxonomy_id = r.term_taxonomy_id
						WHERE x.taxonomy = 'post_format' AND x.term_id IN({$ids_str})
          )";
				} else {
					$where .= " AND p.ID NOT IN (
						SELECT object_id
    ...
    private function __get_most_popular($instance) {
			// Parse instance values
			$instance = $this->__merge_array_r(
				$this->defaults,
				$instance
			...
			if ( !defined('WPP_ADMIN') && $this->user_settings['tools']['cache']['active'] ) {
				...
				return $this->_query_posts( $instance );
			}
    }
      ...
      $mostpopular = $this->__get_most_popular($instance);
		...
		private function __render_popular_post($p, $instance) {
			...
			if ($instance['markup']['custom_html']) {
				...
				$data = array(
					...
					'category' => $post_cat,
					'author' => '<a href="' . get_author_posts_url($p->guid) . '">' . $author . '</a>',
		...
		protected function _get_author($p, $instance) {
			...
			$author = ($instance['stats_tag']['author'])
			  ? get_the_author_meta('display_name', $p->guid)
			  : "";
		...
		protected function _get_date($p, $instance) {
			...
			$date = date_i18n($instance['stats_tag']['date']['format'], strtotime($p->post_date));
		...
		protected function _get_stats($p, $instance) {
			...
			if ($instance['stats_tag']['author']) {
				$author = $this->_get_author($p, $instance);
				$display_name = '<a href="' . get_author_posts_url($p->guid) . '">' . $author . '</a>';
    ...
    private function __get_opts($atts) {
			/**
			* @var String $header
			* @var Int $limit
			* @var String $range
    ...
    /**
     * Get posts as an array of objects
     */
    public function get_posts_array($atts = null) {
      $opts = $this->__get_opts($atts);
      $opts['raw'] = true;

			return $this->__get_most_popular($opts);
    }
      ...
      $shortcode_ops = $this->__get_opts($atts);

			$shortcode_content = "\n". "<!-- WordPress Popular Posts Plugin v". $this->version ." [PHP] [".$shortcode_ops['range']."] [".$shortcode_ops['order_by']."] [custom]" . ( !empty($shortcode_ops['pid']) ? " [PID]" : "" ) . ( !empty($shortcode_ops['cat']) ? " [CAT]" : "" ) . ( !empty($shortcode_ops['author']) ? " [UID]" : "" ) . " -->"."\n";
...
/**
 * Get most popular posts as an array of post object
 **/
function wpp_get_raw_mostpopular($args = NULL) {
  $wpp = new WordpressPopularPosts();

  return $wpp->get_posts_array($args);
}
